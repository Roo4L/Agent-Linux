#!/usr/bin/env bash
set -euo pipefail
# jira-atlassian-mcp uninstall.sh — symmetric inverse (Phase 43, MCP-10).
#
# Deregisters the Atlassian Rovo MCP server from EVERY present MCP-capable agent
...
echo "${server}: deregistering the Atlassian Rovo MCP server from all present agents"
