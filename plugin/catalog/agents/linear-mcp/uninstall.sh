#!/usr/bin/env bash
set -euo pipefail
# linear-mcp uninstall.sh — symmetric inverse (Phase 42, MCP-09).
#
# Deregisters the Linear remote MCP server from EVERY present MCP-capable agent
...
echo "${server}: deregistering the Linear remote MCP server from all present agents"
