#!/usr/bin/env bash
set -euo pipefail
# gitlab-mcp uninstall.sh — symmetric inverse (Phase 38, MCP-05).
#
# Deregisters the GitLab remote MCP server from EVERY present MCP-capable agent
...
echo "${server}: deregistering the GitLab remote MCP server from all present agents"
