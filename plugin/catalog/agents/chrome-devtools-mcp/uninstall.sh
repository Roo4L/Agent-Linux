#!/usr/bin/env bash
set -euo pipefail
# chrome-devtools-mcp uninstall.sh — symmetric inverse (Phase 34, MCP-01 / ENABLE-02).
#
...
# registration is left behind either way.
...
if command -v claude >/dev/null 2>&1; then
  ...
  # filesystem) and avoids a predictable $$ path; a RETURN-less EXIT trap cleans the
  # tmp on any early exit. A parse failure leaves the original untouched (idempotent).
