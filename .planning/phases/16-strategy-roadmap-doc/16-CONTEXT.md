# Phase 16: Strategy + Roadmap Doc - Context
...
Phase 16 lands the canonical product strategy/roadmap document
...
spine; STRATR-02 amends in the same Phase 16 commit window (precedent: Phase 15
...
4. `## What's next` — fused near-term + v0.6+ section (locked 2026-05-19).
   ...
   - `### Near-term` — current-milestone tail + the next 1-2 milestones in
     order: finish v0.3.3 (Phase 17 website refresh) → v0.3.4 brownfield
   ...
   - `### Themes for v0.6+` — 4 themes (locked count; STRATR-03 ceiling).
     ...
     1. Security Hardening (Phase 14 opportunistic theme — capability-scoped
     ...
     2. Preset / profile framework + compat-guarded update flow (Phase 13
...
- **Near-term:** v0.3.3 tail (Phase 17 site refresh) → v0.3.4 brownfield
...
Resolved: Phase 14 verdict (b) declined a separate *Pillar 3* (no honest
...
- Voice-rule grep gate (STRATR-06) — same regex as VIS-07 on VISION.md.
  Hard gate; command + empty output committed verbatim to `16-AUDIT.md`.

### STRATR-02 amendment (Phase 16 commit window)
...
- **Cross-reference / ADR-citation density.** Default to lighter-touch: only
  the load-bearing ADRs (ADR-002, ADR-004, ADR-011, ADR-016) and the two
...
### Phase 15 upstream (consumed by Phase 16)
...
- `docs/decisions/016-agenda-redefinition.md` — ADR-016, records the framing
  decision (two pillars + vision/strategy split). STRATEGY.md cross-links in
  `## Related`.
- `.planning/phases/15-vision-doc-and-downstream/15-CONTEXT.md` — Phase 15
  locked decisions; reference for voice-rule discipline + pillar names.
- `.planning/phases/15-vision-doc-and-downstream/15-AUDIT.md` — Phase 15
  audit; reference for the STRATR-06 grep-gate evidence pattern.

### Substance sources for the 4 themes

- `docs/exploration/PILLAR-2-NOTES.md` — Phase 13 verdict; source of the
...
No code changes in Phase 16 — doc-only.

### Reusable assets

- `docs/VISION.md` (4.4 KB) — voice + structure reference. The committed
  Phase 15 draft sets the canonical pattern: short H2 sections, identity-prose
...
- `docs/decisions/016-agenda-redefinition.md` — recently authored; voice +
...
- Phase-close audit (`16-AUDIT.md`) cites file path + line range / commit
  hash / grep transcript per STRATR-XX requirement. Same pattern as
  `15-AUDIT.md` (Plan 15-02 precedent).
...
- **Phase 17 (Website Refresh)** — consumes `docs/STRATEGY.md` for the
  `#comparison` block reframe (SITE-04) and footer link (SITE-07). Phase
  15 must close before Phase 17 starts.
- **REQUIREMENTS.md STRATR-02 amendment** — same commit window as STRATEGY.md
  drafting. Mirrors Phase 15's mid-milestone REQUIREMENTS.md reframe
  (STRAT-* superseded by VIS-* + STRATR-* on 2026-05-16).
- **`<phase>-AUDIT.md` convention** — Phase 16 emits
  `.planning/phases/16-strategy-roadmap-doc/16-AUDIT.md` with STRATR-01..06
  evidence + GREEN gate.
...
- **`## What we're solving` does NOT duplicate VISION.md.** VISION.md was
  *deliberately* purged of diagnosis content during Phase 15 (user pushback
...
- **`/gsd-complete-milestone` cadence binding** — update the
  ...
  Deferred to v0.3.3 retrospective per ADR-016 Consequences. Pitfall
  #12 / #23 mitigation.
- **Strategy-doc periodic-review cadence** — separate from the cadence
  ...
  every quarter? on substance changes?). Not addressed in Phase 16.
...
- **External-link-rot guard** — Jira links (AL-7, AL-38) and Anthropic CDN
  references in STRATEGY.md could rot. Not addressed in Phase 16; the doc
