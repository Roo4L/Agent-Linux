# Phase 17: Website Refresh (agentlinux.org) - Context
...
Phase 17 repairs `index.html` so it no longer contradicts the post-Phase-14
...
Phase 15 STRAT-* → VIS-* + Phase 16 STRATR-02 reframe).
...
Same commit window as Phase 17 implementation. Mirrors the Phase 15
STRAT-* → VIS-* + Phase 16 STRATR-02 reframe precedent. A
...
| SITE-12 | **KEEP** | Phase-close audit + milestone-close gate fires from this phase (last v0.3.3 phase). Audit cites every SITE-XX evidence (including supersession decisions). Path: `.planning/phases/17-website-refresh-agentlinux-org/17-AUDIT.md` (slug differs from spec's `16-website-refresh` — uses SDK-derived directory name). |
...
### Phase 15 + 15 upstream (consumed by Phase 17)
...
- `.planning/phases/15-vision-doc-and-downstream/15-CONTEXT.md` — locked
  voice-rule discipline + pillar names.
- `.planning/phases/16-strategy-roadmap-doc/16-CONTEXT.md` — locked
  diagnosis voice + the "first usable AgentLinux release for the
  maintainer" framing.
- `.planning/phases/15-vision-doc-and-downstream/15-AUDIT.md`,
  `.planning/phases/16-strategy-roadmap-doc/16-AUDIT.md` — reference
...
- `.planning/REQUIREMENTS.md` — SITE-01..SITE-11 + SITE-12 (Phase 17
  Coverage block at line ~81); the SITE-* amendment lands in this phase.
- `.planning/ROADMAP.md` — Phase 17 entry; the canonical phase boundary.
...
- `docs/decisions/016-agenda-redefinition.md` — ADR-016, the framing
...
- Phase-close audit cites file path + line range / grep transcript per
  SITE-XX requirement. Same pattern as `15-AUDIT.md` / `16-AUDIT.md`.
...
- **REQUIREMENTS.md SITE-* amendment** — same commit window as the HTML
  edits. Mirrors Phase 15 / 15 mid-milestone reframes.
...
- **Milestone-close gate** — Phase 17 is the last v0.3.3 phase. Audit
