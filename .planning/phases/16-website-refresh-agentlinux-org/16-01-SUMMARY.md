...
provides:
  ...
  - 16-AUDIT.md (phase-close + v0.3.3 milestone-close gates both GREEN)
...
key-files:
  created:
    ...
    - .planning/phases/16-website-refresh-agentlinux-org/16-01-SUMMARY.md
  modified:
    - index.html
...
key-decisions:
  ...
  - "REQUIREMENTS.md amendment lands in same commit window as HTML edits (Phase 14 STRAT-* / Phase 15 STRATR-02 precedent); SITE-12 introduced additively via the amendment (mirrors STRATR-07 introduction precedent)"
...
patterns-established:
  ...
  - "Audit-itself defensive voice grep at the end of each AUDIT.md (carries from 15-AUDIT.md): ensures the audit text doesn't introduce voice-rule regressions on itself"
...
# Phase 16 Plan 01: Website Refresh (agentlinux.org) Summary
...
- **Files modified:** 2 (index.html, .planning/REQUIREMENTS.md); 2 created (assets/og-image.png, 16-AUDIT.md); 1 SUMMARY (this file)
...
- `16-AUDIT.md` cites every SITE-XX disposition (PASS / PASS-via-supersession / SUPERSEDED / N/A) and emits both the Phase 16 gate and the v0.3.3 milestone-close gate GREEN. Phase 16 is the last v0.3.3 phase.
...
8. **Task 8: 16-AUDIT.md (Phase 16 + v0.3.3 milestone-close gates GREEN)** — `944ae57` (docs)
...
- `.planning/REQUIREMENTS.md` — Appended `## Superseded Items (2026-05-24 Phase 16 scope re-cut)` block (5th such block in the file) recording SITE-01..SITE-12 dispositions; original SITE-01..SITE-11 + SITE-12 requirement entries unchanged.
- `.planning/phases/16-website-refresh-agentlinux-org/16-AUDIT.md` — New phase-close audit + v0.3.3 milestone-close gate emission.
- `.planning/phases/16-website-refresh-agentlinux-org/16-01-SUMMARY.md` — This file.

## Decisions Made

All key decisions were inherited from `16-CONTEXT.md` `<decisions>` (locked 2026-05-24 during smart-discuss with mid-flow scope re-cut). The execution layer added five carryforward observations:

1. The under-radar posture from STRATEGY.md `## Guiding policy` drives every SITE-* deferral in this phase. Re-evaluation gate: v0.3.4 brownfield installer (AL-38) — at that point the site CTA posture (`#install` snippet, footer doc-links, nav Vision link) warrants a re-look. Phase 16 closure does NOT close those deferred items; they continue to wait on the AL-38 gate.
...
- **Fix:** None applied. Both occurrences align with STRATEGY.md `## Our bets` (the curated-combo bet) and 16-CONTEXT.md `<decisions>` rule (iv) (curated-combo bet surfaces in feature cards + comparison blocks).
- **Files modified:** None this deviation.
...
- **Fix:** None applied. Three occurrences align on the canonical site-side framing per 16-CONTEXT.md `<decisions>` ("installable Ubuntu plugin" is the canonical phrase for what AgentLinux is).
- **Files modified:** None this deviation.
...
**4. [Rule 1 - Bug] Initial 16-AUDIT.md authoring included a reviewer-pass rubric row that literally quoted the SITE-06 forbidden verbs (`AgentLinux benchmarks/measures/defends/...`), tripping the audit-itself defensive voice-rule grep.**
...
- **Files modified:** `.planning/phases/16-website-refresh-agentlinux-org/16-AUDIT.md` (one-line rephrase before commit).
- **Verification:** Post-rephrase audit-itself defensive grep returns empty stdout + exit=1; full Task 8 verify chain passes (`TASK 8 PASS — Phase 16 + v0.3.3 milestone-close gates GREEN`).
...
**v0.3.3 milestone closed.** All 5 v0.3.3 phases (12, 13, 14, 15, 16) carry GREEN audits at the canonical paths. Per the v0.3.3 milestone-close gate emission in `16-AUDIT.md`, the milestone is complete.
