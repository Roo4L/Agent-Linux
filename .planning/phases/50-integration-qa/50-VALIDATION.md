...
| Productive stop decision | TST-08 | Productive activity and novelty are session observations, not wall-clock chat time | Log active intervals only; reset both measures for each new reproducible issue; stop only when active time ≥30 minutes and the latest 10 distinct ideas are clean for new-issue discovery. Known-issue replays are neither new nor clean and do not advance the clean counter. |
