...
| Docker behavior suite | 335/336 pass | `bash tests/docker/run.sh ubuntu-24.04`; WIRE-02 test 336 passes. |
...
2. `plugin/catalog/lib/rtk-wire.sh` only unwired targets whose agent binary was
   still present. Removing Codex first therefore left RTK-owned files in the
   preserved Codex config. Unwire now also recognizes RTK-owned artifacts; the
   new WIRE-02 behavior test passes in Docker and in the disposable RC sandbox.
