# Phase 12: Pillar 2 Exploration - Context
...
verdict (`docs/exploration/PILLAR-2-NOTES.md`) that Phase 14 lifts verbatim into
...
Phase 12 decides the *content* under that name.
...
### Reusable Assets (no code changes in Phase 12 — doc-only)
...
- Voice rule (Pitfall, applied here too): every claim about an unshipped
  ...
  this rigorously.
- Phase-close audit convention: `.planning/phases/12-pillar-2-exploration/12-AUDIT.md`
  cites file path + line range of Decision summary + grep transcript; gate
  emits GREEN before phase closes.

### Integration Points

- Phase 14 (Strategy Doc) lifts the Decision summary verbatim into
  `docs/STRATEGY.md` Pillar 2. The Decision summary section heading
  (`## Decision summary`) is Phase 14's grep anchor — must be exact.
- Phase 13 (Pillar 3 Candidate Exploration) reads this CONTEXT.md as prior
  context to avoid re-deciding pillar boundaries.
- Phase 15 (Website Refresh) consumes the Today / Direction split for the
...
- **Pillar-3 substance** — entirely deferred to Phase 13.
