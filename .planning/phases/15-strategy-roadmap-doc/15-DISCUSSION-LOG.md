# Phase 15: Strategy + Roadmap Doc - Discussion Log
...
| Current-milestone tail only | Just Phase 16 website refresh | |
...
**Notes:** Phase 15 references AL-38 by Jira key only; does not claim authoritative in-repo plan state.
...
- Phase 13 verdict (b) declined Security as a separate Pillar 3 (no honest already-shipped table-stakes).
- Phase 13 explicitly *kept* Security Hardening as a v0.6+ *opportunistic theme* (declined defenses eligible to mature into milestones later).
...
**Notes:** Major mid-discuss reframe. REQUIREMENTS.md STRATR-02 amends in the same Phase 15 commit window. Precedent: Phase 14 STRAT-* → VIS-* + STRATR-* reframe 2026-05-16.

### Round 3 — Diagnosis vs duplication of VISION.md

| Option | Description | Selected |
|--------|-------------|----------|
| Keep `## Diagnosis` with one-line VISION.md reference + specific bug-class content | No duplication risk (VISION.md was deliberately purged of diagnosis content during Phase 14) | ✓ |
...
**Notes:** Confirmed VISION.md does not contain diagnosis content. The narrow problem-list (EACCES, recursive shim, self-update breakage) was deliberately cut from VISION.md Mission during Phase 14 to keep it broad-value-prop.
...
- Only load-bearing ADRs (ADR-002, ADR-004, ADR-011, ADR-015) + the two exploration verdict files get inline links inside section bodies.
