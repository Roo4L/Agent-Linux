...
- [x] **AGT-02b**: Installing Claude Code via `agentlinux install claude-code` produces exactly `pinned_version` on disk — `claude --version` matches the catalog's `pinned_version` field — verifying the version-lock mechanism from ADR-011 works end-to-end. Companion test to AGT-02 (which is version-agnostic). (Verified 05-01 Docker smoke on Ubuntu 22.04 + 24.04: in-recipe `grep -q -F -- "${AGENTLINUX_PINNED_VERSION}"` against `claude --version` output inside plugin/catalog/agents/claude-code/install.sh; install fails fast if upstream bootstrap drifts from the pin. Bats-side assertion lands in Plan 05-04's 50-agents.bats.)
- [ ] **AGT-03**: `claude doctor` (or equivalent diagnostic) reports a clean state for the agent user after install.
- [~] **AGT-04**: After `agentlinux install gsd`, the agent user can run `gsd --version` (or equivalent) successfully. (Recipe body ✓ Complete 05-02: plugin/catalog/agents/gsd/install.sh `npm install -g get-shit-done-cc@${PINNED} --omit=dev --no-fund --no-audit` + post-install `command -v get-shit-done-cc` PATH-resolve check + banner-grep version-lock via `get-shit-done-cc --help | head -20 | grep -q -F v${PINNED}` — no `--version` flag exists per `npm view get-shit-done-cc bin`, so `--help` banner-grep is the version-lock mechanism. Binary is `get-shit-done-cc` (NOT `gsd`); no symlink added per research §Open Question 1 (would be a hidden shim, anti-pattern). End-to-end exercised on host: 0.542s install wall-time, banner matches pin, uninstall idempotent. Bats-side assertion (end-to-end `agentlinux install gsd` + verify command) lands Plan 05-04's 50-agents.bats.)
- [~] **AGT-05**: After `agentlinux install playwright`, the agent user can run `npx playwright --version` and `npx playwright install` (downloads browsers into the agent user's cache, no sudo, no EACCES). Playwright is the canonical browser-access tool for agents (replaces v0.2.0's chrome-devtools-mcp). (Recipe body ✓ Complete 05-03: plugin/catalog/agents/playwright/install.sh 79-line 3-part body — `npm install -g playwright@${PINNED} --omit=dev --no-fund --no-audit` + `command -v playwright` PATH-resolve check + `playwright --version | grep -q -F -- "${PINNED}"` pre-download version-lock + `npx --yes playwright install --with-deps chromium` one-shot (downloads ~281 MB chromium + runs apt-install-deps via Playwright's internal sudo auto-prepend, non-interactive via ADR-012 NOPASSWD drop-in) + post-install `find ${cache_dir} -name 'chromium-*'` assertion. Symmetric uninstall.sh 23 lines: `npm uninstall -g playwright` + `rm -rf ~agent/.cache/ms-playwright` (~631 MB reclaimed) + `command -v` absence check. Chromium-only (no firefox/webkit) per Phase 5 deferred scope. End-to-end smoke on Ubuntu 22.04 + 24.04: exit 0, pw_version=1.59.1, chromium-1217 cache agent-owned, 98-102 apt packages installed, zero "password is required" lines (ADR-012 sentinel green). Bats-side end-to-end assertion lands Plan 05-04's 50-agents.bats.)
...
- [x] **TST-01** (partial — grows with each phase): A black-box behavior-test suite exists that covers every `BHV-XX`, `RT-XX`, `AGT-XX`, `CLI-XX`, `CAT-XX`, and `INST-XX` requirement with at least one automated test. (Phase 2 portion ✓: 22 @tests in tests/bats/10-installer.bats + tests/bats/20-agent-user.bats covering INST-01/02/05 + BHV-01..06 + DOC-02. Phase 3 ✓: +5 @tests in tests/bats/30-runtime.bats covering RT-01..04. Phase 4 ✓: +22 @tests in tests/bats/40-registry-cli.bats covering CLI-01..07 + CAT-01..04 + INST-04 (Plan 04-07, 49/49 green on Ubuntu 22.04 + 24.04). Suite at 49 @tests; grows in Phase 5 with AGT-XX. TST-01 will be fully [x] when Phase 5 closes.)
...
| AGT-02b | Phase 5 | ✓ Complete (05-01 recipe: plugin/catalog/agents/claude-code/install.sh `grep -q -F -- "${AGENTLINUX_PINNED_VERSION}"` in-recipe assertion; fail-fast on upstream bootstrap drift; AGT-02b bats-side assertion lands in Plan 05-04's 50-agents.bats) |
| AGT-03 | Phase 5 | Pending |
| AGT-04 | Phase 5 | Recipe ✓ (05-02: plugin/catalog/agents/gsd/install.sh 51-line npm-global body with `--omit=dev --no-fund --no-audit` + `command -v get-shit-done-cc` resolve check + `--help` banner-grep version-lock; symmetric uninstall.sh 19 lines with idempotent `\|\| true` + `command -v` absence check. Bats-side end-to-end assertion lands Plan 05-04's 50-agents.bats.) |
| AGT-05 | Phase 5 | Recipe ✓ (05-03: plugin/catalog/agents/playwright/install.sh 79-line 3-part body — `npm install -g playwright@${PINNED}` + `command -v playwright` resolve check + `playwright --version` pre-download grep -F pin verification + `npx --yes playwright install --with-deps chromium` one-shot (~281 MB chromium via Playwright CDN + apt install-deps via ADR-012 NOPASSWD non-interactive sudo auto-prepend) + post-install `chromium-*` cache-dir assertion. Symmetric uninstall.sh 23 lines: `npm uninstall -g playwright \|\| true` + `rm -rf ~agent/.cache/ms-playwright` + `command -v` absence check. End-to-end smoke on Ubuntu 22.04 + 24.04 both green; observed pw_version=1.59.1, chromium-1217 cache agent-owned, 98-102 apt Setting-up lines, zero "password is required" (ADR-012 sentinel green). Bats-side end-to-end assertion lands Plan 05-04's 50-agents.bats.) |
