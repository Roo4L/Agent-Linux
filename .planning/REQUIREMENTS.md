...
| CLI-01 | Phase 4 | ✓ Complete (04-06: plugin/provisioner/50-registry-cli.sh stages CLI bundle trio dist+node_modules+package.json under /opt/agentlinux/cli/0.3.0/; symlinks /home/agent/.npm-global/bin/agentlinux → dist/index.js via ln -sfn + chown -h; Docker smoke on Ubuntu 22.04 + 24.04 confirms `agentlinux --version`=0.3.0 as agent user in login + SSH + systemd + cron modes; `agentlinux list` prints 3-agent table. Bats enforcement lands Plan 04-07.) |
...
| CLI-06 | Phase 4 | ✓ Complete TS-side (04-04: upgradeCmd with 5-opt UpgradeOpts + 3-call UpgradeDeps DI; plugin/cli/src/upgrade/{divergence,npm_ls}.ts — pure classifier computeDivergence + resolveLatestFor with semver.maxSatisfying honoring version_constraint; queryGlobalNpm Pitfall-4-safe defensive parse; queryNpmViewLatest with single-string-vs-array coercion; willTouchUpstream() offline-default gate; report-only default; sticky entries skipped unless --reset-all-curated per ADR-011; sticky preservation on --all-latest; per-entry upstream errors non-fatal; recipe-failure preserves prior sentinel; 38 unit tests — 92/92 total green; bats integration lands Plan 04-07) |
| CLI-07 | Phase 4 | ✓ Plan 04-05 (TypeScript side: pin.ts pinCmd + parsePinSpec + PinTarget; 20 unit tests; bats enforcement Plan 04-07) |
...
| INST-04 | Phase 4 | ✓ Complete (04-06: plugin/bin/agentlinux-install --purge replaces Phase 2 stub with 7-step ordered idempotent teardown; per-agent uninstall.sh → rm -rf /opt/agentlinux → PATH artefacts → NodeSource apt → optional apt-purge nodejs gated on --remove-nodejs → pkill+userdel -r agent → log-removal LAST (Pitfall 7 tee-EOF); T-04-16 + T-04-17 mitigations; all rm targets literal absolute paths. Bats enforcement lands Plan 04-07.) |
