# Phase 15: Vision Doc + ADR-016 + Downstream Surface Updates - Context
...
Phase 15 lands the canonical product-vision document (`docs/VISION.md`),
records the framing decision in ADR-016, and propagates the new framing to
...
**Major reframe locked 2026-05-16:** The original Phase 15 (per ROADMAP at
...
named `docs/VISION.md`. The strategy/roadmap content moves to a new Phase 16
(`docs/STRATEGY.md`). Website refresh renumbers from Phase 16 → Phase 17.
...
- **Guiding principles** — 4–6 vision-level principles, each a `### {Name}` +
  ...
  - OUT: "Behavior tests are the spec" (ADR-002); "TST-07 phase-close
    ...
    principles and live in Phase 16's STRATEGY.md `## Execution principles`.
...
The doc does NOT cover (these moved to Phase 16's STRATEGY.md):
...
Phase 14 verdict (b) means there is no Pillar 3. The vision doc ships
exactly 2 pillars.
...
command + empty output gets committed verbatim to `15-AUDIT.md`.

### ADR-016 — agenda redefinition (VIS-09)

`docs/decisions/016-agenda-redefinition.md` lands in the same Phase 15
...
- `Decision` — Two pillars (locked by Phase 14 verdict (b)); vision-only
...
- `Considered alternatives` (≥3):
  ...
  2. Ship vision + strategy + roadmap + trade-offs in one `docs/STRATEGY.md`
     per the original Phase 15 plan (rejected — user reframe 2026-05-16;
     vision and strategy serve different audiences and benefit from doc-
     level separation).
  3. Pivot security-first to a Pillar 3 (rejected — Phase 14 verdict (b);
     no honest already-shipped table-stakes for security as a pillar; would
     force aspirational drift per Pitfall #6).
- `Consequences` — Phase 16 (strategy/roadmap doc) inserted; Phase 16 →
  Phase 17 renumber of website-refresh; DOC-05 closes N/A; Pillar 2
  carries the supply-chain monitoring sub-concern from Phase 14 verdict (b);
...
Phase 14 verdict (b) means no Pillar 3 → no ADR-012 forward-reference
...
- fact-checker: all claims verified against Phase 13 + Phase 14 locked
...
- Section ordering inside ADR-016 considered-alternatives (which of the
...
- Whether to add a one-line `### Reviewer notes` block at the bottom of
  `15-AUDIT.md` summarizing the reviewer pass (not required by VIS-* but
  helpful audit trail).
...
### Reusable Assets (no code changes in Phase 15 — doc-only)
...
- `docs/exploration/PILLAR-2-NOTES.md` — Phase 13 verdict, lifted into VISION.md
  Pillar 2 substance.
- `docs/exploration/PILLAR-3-CANDIDATE-NOTES.md` — Phase 14 verdict (b),
  lifted into the supply-chain sub-concern of VISION.md Pillar 2 and into
  the non-goals list.
- `docs/decisions/000-template.md` — ADR template, used to author ADR-016.
...
- Phase-close audit convention: `<phase-NN>-AUDIT.md` cites file path + line
  ...
  before phase closes.
- ADR numbering: ADR-016 is the next slot.
- Reviewer pass on docs: technical-writer + fact-checker before commit.
  Already run on `docs/VISION.md` 2026-05-16.

### Integration Points

- **Phase 16 (Strategy + Roadmap Doc):** authors `docs/STRATEGY.md` with
  the execution principles + themes-for-v0.6+ + current-focus content that
  was cut from Phase 15. Phase 16 starts after Phase 15 closes.
- **Phase 17 (Website Refresh):** consumes both VISION.md (for pillar names
  + non-goals) and STRATEGY.md (for "Where we are now" framing in the
  `#comparison` block reframe).
- **AL-14 (Jira anchor Task):** Phase 15 status updates land as comments on
  AL-14 or as a new Subtask under AL-14 alongside AL-40 (Phase 13) and AL-42
  (Phase 14).
...
- **The Mission paragraph is broad on purpose.** Don't reintroduce the
  ...
  ADR-016 authoring or DOC-01 README updates — the user explicitly
...
- **DOC-05 N/A close cites EXPL-02's `## Verdict` line.** Audit must
  ...
  N/A close without re-reading Phase 14.
- **The vision/strategy split is the load-bearing ADR-016 decision.**
  ADR-016's Considered alternatives must include "ship vision + strategy
...
- **Strategy/roadmap content** (execution principles, themes for v0.6+,
  near-term focus, current state) — moved to Phase 16 (STRATEGY.md).
