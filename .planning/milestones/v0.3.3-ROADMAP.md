...
**Phase numbering:** Continues temporally from v0.4.0 (last phase 11) → v0.3.3 starts at **Phase 13**. The version number reverted from v0.4.0 → v0.3.3 per user re-numbering, but phase numbering follows real chronology so phase directories are unique. v0.3.0 directories (`.planning/phases/01-*..06-*`) and v0.4.0 directories (`.planning/phases/07-*..11-*`) remain in place; v0.3.3 phases land at `.planning/phases/13-*..17-*` alongside them (Phase 12 is the post-v0.4.0 AL-22 developer-internals addendum).
...
1. **Pillar count is not pre-decided.** AL-7 proposed three pillars (env + stability + security). Phase 14's verdict on "is security a pillar?" decided the framing lands with 2 pillars (verdict (b) — fold into pillar 2). Phases 14+ consume that verdict.
2. **Exploration phases come FIRST.** Phases 13 and 14 are exploration/decision phases that produce written conclusions docs (`docs/exploration/PILLAR-*-NOTES.md`). Downstream phases consume those conclusions.
3. **Phase 13 → Phase 14 is sequential.** User direction: pillar 2 dug into first; pillar 3 (security) second.
4. **Vision and strategy are separate documents.** User reframed Phase 15 mid-milestone (2026-05-16): the original Phase 15 bundled vision + strategy + roadmap + framework trade-offs into one `docs/STRATEGY.md` against the Sourcegraph template. New shape: Phase 15 produces `docs/VISION.md` (vision-only — mission, pillars, principles, non-goals); a new Phase 16 produces `docs/STRATEGY.md` (strategy/roadmap — execution rules, sequencing, themes for v0.6+, current focus). Website refresh renumbers from Phase 16 → Phase 17.

The roadmap lands at **5 phases** (13 → 14 → 15 → 16 → 17). Phase 15's reframe + Phase 16 insertion locked 2026-05-16.
...
- `docs/STRATEGY.md` is the canonical "how we get there" reference: execution rules (voice rule, behavior-tests-as-spec, evidence-cite discipline), theme sequencing for v0.6+, near-term focus. Lands AFTER Phase 15 so it can reference VISION.md as upstream. Target size ≤ 8 KB.
...
- DOC-05 closes N/A: Phase 14 verdict (b) means no Pillar 3, no ADR-012 forward-reference edit needed.
...
**Phase Numbering:**
- Integer phases (13, 14, 15, 16, 17): Planned milestone work, executed in numeric order
- Decimal phases (e.g., 14.1) reserved for urgent insertions discovered during the milestone (precedent: v0.3.0 Phase 5.1)
...
**Plans**:
- [x] 13-01-PLAN.md — Author `docs/exploration/PILLAR-2-NOTES.md`; produce Decision summary; phase-close audit (EXPL-01)

### Phase 14: Pillar 3 Candidate Exploration
**Goal**: Decide whether security is a pillar at all. Produce a written verdict.
**Depends on**: Phase 13 (sequential per user direction)
...
**Plans**:
- [x] 14-01-PLAN.md — Author `docs/exploration/PILLAR-3-CANDIDATE-NOTES.md`; produce Verdict + Decision summary; phase-close audit (EXPL-02)

### Phase 15: Vision Doc + ADR-016 + Downstream Surface Updates
**Goal**: Land the canonical product-vision document (`docs/VISION.md`), record the framing decision in ADR-016, and propagate the new framing to every downstream documentation surface so a future visitor reading any of {README, CONTRIBUTING, PROJECT.md, STABILITY-MODEL.md, VISION.md} sees the same coherent two-pillar story without contradictions or stale references. Voice-rule grep gate enforced as a phase-close hard gate.
**Depends on**: Phase 13 (consumes EXPL-01 pillar-2 verdict into Pillar 2 framing) + Phase 14 (consumes EXPL-02 verdict to set pillar count = 2)
...
**Success Criteria** (what must be TRUE):
  ...
  3. The Pillars section contains exactly 2 pillars (matching the Phase 14 (b) verdict). `grep -cE '^### Pillar [0-9]+' docs/VISION.md` returns exactly 2. Pillars are named by the optimization value (e.g. `### Pillar 1 — Time-to-productive`, `### Pillar 2 — Stability`). No `#### Today` / `#### Direction` subsections inside pillars (vision-doc voice, not status-report voice). — VIS-03.
  ...
  7. **Voice-rule grep gate (HARD GATE).** `grep -nE '^[^a-z]*AgentLinux (provides|offers|ensures|protects|defends|benchmarks|measures|hardens|isolates|detects|prevents)\b' docs/VISION.md` returns zero matches. The exact command + its empty output is committed verbatim to `.planning/phases/15-vision-doc-and-downstream/15-AUDIT.md`. Phase-close gate fails on any match. — VIS-07.
  8. Cross-link map populated. Outbound: pillar / principle / non-goal claims in VISION.md that ground in an ADR may carry a Markdown link (light hand — vision-voice keeps most claims abstract). Inbound: each of `README.md` (About + Links), `CONTRIBUTING.md`, `.planning/PROJECT.md` (Core Value section), `docs/STABILITY-MODEL.md` (Related section) carries a back-pointer to VISION.md. Phase-15 audit lists each changed file with the line range of the back-pointer edit. — VIS-08.
  9. `docs/decisions/016-agenda-redefinition.md` (ADR-016) lands in the same milestone window as VISION.md (same Phase 15 commit window). The ADR contains: `Status: Accepted`, `Context` (the AL-7 framing question + why the original single-pillar framing was getting in the way), `Decision` (the two-pillar landing — citing the EXPL-01 + EXPL-02 verdicts; vision-only document separated from strategy/roadmap), ≥ 3 considered-and-rejected alternatives (e.g. "stay single-pillar," "ship vision+strategy+roadmap in one doc per original Phase 15 plan," "pivot security-first to a Pillar 3"), `Consequences` (including the Phase 16 strategy-doc insertion + Phase 16 → Phase 17 renumber of the website-refresh phase), and a back-link to AL-7 + VISION.md. `grep -cE '^## (Status|Context|Decision|Considered alternatives|Consequences)' docs/decisions/016-agenda-redefinition.md` returns ≥ 5. — VIS-09.
  10. **DOC propagation.** Each of these files shows a commit in the Phase 15 window touching the named section: `README.md` (About + Links sections gain pillar-naming sentence + Vision link — DOC-01); `CONTRIBUTING.md` ("Why this project exists" paragraph + pillar-status callout — DOC-02); `.planning/PROJECT.md` (Core Value + Current Milestone sections cross-reference VISION.md; Out-of-Scope reflects EXPL-01/02-surfaced non-goals — DOC-03); `docs/STABILITY-MODEL.md` (Related section with back-link to VISION.md Pillar 2 — DOC-04). Phase-close audit cites the commit hash + line range for each. — DOC-01..04.
  11. **DOC-05 closes N/A.** Phase 14 verdict (b) — pillar 3 did NOT survive. No edit to `docs/decisions/012-agent-user-full-sudo.md` (ADR-012) needed. Phase 15 audit records DOC-05 as N/A with a one-line decision: "no edit needed because pillar 3 did not survive Phase 14"; audit explicitly cites EXPL-02's `## Verdict` line. — DOC-05.
  12. Phase-close audit `.planning/phases/15-vision-doc-and-downstream/15-AUDIT.md` cites every VIS-XX + DOC-XX evidence (file path / line range / commit hash / grep transcript per requirement); gate emits GREEN.
**Plans**: 2 plans
- [ ] 15-01-PLAN.md — Verify committed `docs/VISION.md` against VIS-01..VIS-07 (capture transcripts in `15-01-EVIDENCE.md`); author ADR-016 (VIS-09); reviewer pass (VIS-01..07, VIS-09)
- [ ] 15-02-PLAN.md — Downstream surface updates: README + CONTRIBUTING + PROJECT.md + STABILITY-MODEL.md back-pointers (VIS-08, DOC-01..04); DOC-05 N/A close; Phase 15 phase-close audit `15-AUDIT.md` (gate emits GREEN)

### Phase 16: Strategy + Roadmap Doc
...
**Depends on**: Phase 15 (consumes VISION.md as the upstream "what"; cannot author "how to achieve the vision" before the vision is locked)
...
**Success Criteria** (what must be TRUE):
  ...
  3. The `## Themes for v0.6+` section lists 2–4 forward-looking themes — at minimum: `Security Hardening` (Phase 14 opportunistic theme — capability-scoped sudoers, cosign-signed catalog releases, npm provenance, etc.); preset/profile framework + compat-guarded update flow (Phase 13 differentiators). Each theme has a `### Sequencing rationale` line. — STRATR-03.
  ...
  6. **Voice-rule grep gate (HARD GATE).** `grep -nE '^[^a-z]*AgentLinux (provides|offers|ensures|protects|defends|benchmarks|measures|hardens|isolates|detects|prevents)\b' docs/STRATEGY.md` returns zero matches. Command + output committed verbatim to `.planning/phases/16-strategy-roadmap-doc/16-AUDIT.md`. Phase-close gate fails on any match. — STRATR-06.
  7. Phase-close audit `.planning/phases/16-strategy-roadmap-doc/16-AUDIT.md` cites every STRATR-XX evidence; gate emits GREEN.
**Plans**: 2 plans
- [x] 16-01-PLAN.md — Amend REQUIREMENTS.md STRATR-02 (Rumelt-style 5-section spine + ≥ 5 grep gate; append 2026-05-19 Superseded-items block); author `docs/STRATEGY.md` from VISION.md + Phase 13 + Phase 14 substance with locked 5-section spine + 4 themes + 5-7 execution principles; voice-rule grep gate; capture 16-01-EVIDENCE.md (STRATR-01..06)
- [x] 16-02-PLAN.md — Reviewer pass on docs/STRATEGY.md (technical-writer + fact-checker, parallel; mirror Phase 15 / docs/VISION.md `f95a4ee` precedent); apply triaged feedback; author phase-close audit `16-AUDIT.md` (STRATR-01..06 lifted verbatim from 16-01-EVIDENCE.md; Phase 16 GREEN gate emission)

### Phase 17: Website Refresh (agentlinux.org)
**Goal**: Repair the agentlinux.org landing page so its framing matches the post-Phase-14 vision (two pillars), the post-Phase-15 strategy, the v0.3.0/v0.4.0 product reality, and the locked pillar count. The site is currently two pivots stale (hero says "purpose-built Linux distribution"; features advertise QEMU/Docker micro-VM distribution) — refreshing it is required regardless. Voice-rule grep gate applies to the rendered HTML, with the same hard-gate semantics as Phase 15 VIS-07 / Phase 16 STRATR-06. Visual redesign is explicitly out of scope; the dark JetBrains Mono aesthetic + crab mascot stay.
**Depends on**: Phase 15 (so the site can link to a stable `docs/VISION.md` URL) + Phase 16 (so the site can link to a stable `docs/STRATEGY.md` URL)
...
**Success Criteria** (what must be TRUE):
  ...
  12. Phase-close audit `.planning/phases/17-website-refresh/17-AUDIT.md` cites every SITE-XX evidence; gate emits GREEN. Milestone-close gate also fires from this phase (last v0.3.3 phase).
...
- [x] 17-01-PLAN.md — `index.html` IA restructure (#pillars replacing #features, hero rewrite, footer + nav additions, OG/Twitter rewrite); OG image PNG conversion + commit; deploy-time anti-drift check wiring; voice-rule grep gate run + committed; mobile screenshots in PR body; Phase 17 phase-close audit + milestone-close gate (SITE-01..11)
**UI hint**: yes

## Progress

**Execution Order:**
Phases execute in numeric order: 13 → 14 → 15 → 16 → 17. Phase 13 → Phase 14 is sequential (user direction). Phase 15 depends on Phases 13 + 14. Phase 16 depends on Phase 15. Phase 17 depends on Phases 15 + 16.
...
| 14. Vision Doc + ADR-016 + Downstream Surface Updates | 2 | In progress | Reframed from "Strategy Doc..." on 2026-05-16; vision-doc draft committed; ADR-016 + downstream propagation pending |
...
**Total v0.3.3 requirements:** 33 (2 EXPL + 14 Phase 15 (VIS+DOC) + 6 Phase 16 (STRATR) + 11 Phase 17 (SITE))
**Mapped:** 33 / 33
**Orphaned:** 0
**Duplicated across phases:** 0

> **Note on requirement-ID changes (2026-05-16):** The original ROADMAP referenced STRAT-01..STRAT-13 + DOC-01..DOC-05 in Phase 15 (18 reqs). The 2026-05-16 reframe split that into Phase 15 (VIS-01..VIS-09 + DOC-01..DOC-05 = 14 reqs, vision-only) and a new Phase 16 (STRATR-01..STRATR-06 = 6 reqs, strategy/roadmap doc). The Sourcegraph-framework-spine requirements (old STRAT-02 spine, STRAT-07 framework trade-offs, STRAT-08 Vision Board, STRAT-09 Appendix B placement-inside-strategy-doc) all dissolved because the doc structure is no longer the Sourcegraph template. The original Phase 16 (Website Refresh) renumbered to Phase 17 with no requirement-ID changes (SITE-01..SITE-11 unchanged).
...
| 14 Vision Doc + ADR-016 + Downstream Surface Updates | VIS-01..VIS-09, DOC-01..DOC-05 | 14 |
...
- Most v0.3.3 work is documentation. Evidence is documentation artifacts under `docs/exploration/PILLAR-*-NOTES.md`, `docs/VISION.md`, `docs/STRATEGY.md`, `docs/decisions/016-agenda-redefinition.md`, plus per-phase audit files. No new bats @tests are required.
...
- Phase 17's only "real product code" change is HTML/CSS/JS in `index.html` + `assets/`. The existing GitHub Pages auto-deploy pipeline (v0.1.0) carries it.
...
- **Voice-rule grep gates (VIS-07 in Phase 15, STRATR-06 in Phase 16, SITE-06 in Phase 17) are the load-bearing structural defense for the milestone.** They are run as part of the phase-close audit and their command + output is committed verbatim to the audit file. The grep is the spec.
- **Pillar 3 conditional handling is resolved.** Phase 14 verdict (b) means no Pillar 3. Downstream conditionals: (i) the `### Pillar 3` section in VISION.md does not exist; (ii) DOC-05 closes N/A in Phase 15 audit; (iii) the `#pillars` section on the website carries 2 cards; (iv) ADR-012 forward-reference is not added.

---

*Last updated: 2026-05-16 — v0.3.3 ROADMAP rewritten for Phase 15 vision-only reframe + Phase 16 strategy-doc insertion + Phase 16 → Phase 17 website-refresh renumber.*
...
### Phase 13: Developer documentation for installer, runtime, and CLI (AL-22)

**Goal:** A reader landing on the AgentLinux repo can find a 60-second answer to "what value does AgentLinux provide for surface X" for every component (installer, agent user, sudo drop-in, Node.js runtime, the agent catalog, the registry CLI, and the curated agent set: Claude Code, GSD, Playwright). The docs stay in sync with the source via a project-scoped reviewer (`dev-docs-auditor`) embedded in the existing review loop — no new stop-hook is added (ADR-016 lands in Plan 13-05).
**Requirements**: DOC-01, DOC-02, DOC-03, DOC-04, DOC-05, DOC-06, DOC-07
**Depends on:** Phase 11
**Plans:** 5/5 plans complete

Plans:
- [x] 13-01-PLAN.md — docs/internals/ index + 4 install/runtime layer component docs (DOC-01, DOC-02)
- [x] 13-02-PLAN.md — 5 agent + CLI/catalog component docs (DOC-02)
- [x] 13-03-PLAN.md — dev-docs-auditor reviewer agent + dev-docs skill (DOC-03, DOC-04, DOC-06)
- [x] 13-04-PLAN.md — CLAUDE.md Review Loop + Pointers wiring + top-level README.md discoverability (DOC-03, DOC-05)
- [x] 13-05-PLAN.md — REQUIREMENTS.md DOC-XX entries + ADR-016 + Phase 13 AUDIT (DOC-01..DOC-07, phase-close) (completed 2026-05-10)
