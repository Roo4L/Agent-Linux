...
- [x] **ENABLE-08**: **Passive autoupdate freeze** — for catalog tools that *auto-install* updates in the background (not merely notify), the install recipe disables that passive self-update via the tool's own **launch-mode-independent** config, so the catalog pin is never silently replaced out of band. The **explicit, user-initiated** update path (`agentlinux upgrade`, the tool's own `upgrade` command, or an npm reinstall) must stay functional — only the passive path is frozen. This applies to opencode and qwen-code in the current catalog. Antigravity CLI exposes `agy update` and upstream background self-updating, so AgentLinux provisions the upstream-supported `AGY_CLI_DISABLE_AUTO_UPDATE=true` variable in login, systemd, and cron environments; the initial binary is still pinned and checksum-verified, and AgentLinux never invokes `agy update`. Codex remains notify-only (`check_for_update_on_startup=false`), and ccusage has no updater.
...
- [x] **WIRE-01**: A catalog entry that installs **skills/commands** for one coding agent must wire them into **every other shipped coding agent for which the concept applies** — so a tool installed via AgentLinux is lit up across the whole installed agent fleet, not just Claude Code. **Order-independent**: the wiring is applied unconditionally at the *provider's* install time (writing each target agent's own config dir), so an agent installed *later* still finds the skills present; `remove` tears the wiring down symmetrically across all targets. Applicability is classified per (provider, target): **DIRECT** (drop or convert into the target's extension dir) or **N/A** (target has no comparable extension host — documented, never silently skipped). Current providers:
  - **GSD** (`@opengsd/gsd-core`, a natively multi-runtime workflow system): wired into Claude Code (`~/.claude/skills/gsd-*`), OpenCode (`~/.config/opencode/skills/gsd-*`), Codex (`~/.agents/skills/gsd-*`), and Qwen Code (`~/.qwen/skills/gsd-*`). Antigravity CLI wiring is not currently provided by upstream; use its migration/import flow manually if needed.
  - **playwright-cli**: skill mirrored into `~/.agents/skills/playwright-cli/` for Codex/OpenCode. Antigravity uses workspace-local `.agents/skills/`, so AgentLinux leaves that workspace-specific copy to the user. Qwen-code remains **N/A** (no compatible multi-file skill scan path).
...
- [x] **WIRE-02**: A catalog entry that wires a **cross-agent proxy/service into each agent's own config** — as opposed to WIRE-01's skill/command files — must converge to the **same wired set regardless of install order**, in **both** directions: (a) installing the *provider* fans its wiring out to every coding agent present *then*; (b) installing a coding agent *later* re-wires every already-installed provider into the *new* agent. Direction (b) is the gap WIRE-01's unconditional-write trick cannot cover here, because these providers edit each agent's *live* config (which needs the agent present) rather than dropping a file. Mechanism: the provider declares `rewire_recipe_path`, and the CLI's post-install reconcile (`plugin/cli/src/rewire.ts`) re-runs each installed provider's rewire recipe after a coding agent is installed. All wiring is **best-effort** (a per-agent hiccup never fails the triggering install) and torn down symmetrically on `remove`. Current providers:
  - **rtk**: `rtk init` hook fanned out to claude-code (`~/.claude/RTK.md` + settings.json hook), codex (`~/.codex/RTK.md` + `AGENTS.md`), and opencode (`~/.config/opencode/plugins/rtk.ts`). Antigravity CLI and qwen-code: **N/A** (rtk ships no target). Helper: `plugin/catalog/lib/rtk-wire.sh`; reverse-trigger recipe: `agents/rtk/rewire.sh`.
...
### Coding-agent CLIs (mixed npm/binary)
...
- [x] **AGT-06**: `agentlinux install antigravity-cli` installs Google's Antigravity CLI (official SHA-512-verified binary, bin `agy`); the initial artifact pin is enforced and the upstream-supported passive-update opt-out is provisioned; symmetric remove preserves `~/.gemini` user state.
...
- [x] **MCP-07**: `agentlinux install firecrawl-mcp` registers Firecrawl MCP into every installed MCP-capable agent as a **bare OAuth hosted remote-http URL** `https://mcp.firecrawl.dev/v2/mcp` (ADR-017 thin installer; prefer-hosted supersedes the roadmap's npx-stdio plan); NO credential baked (`requires_secret: true`, no `secret_env`); `install` prints the client-owned OAuth/API-key fallback pointer; symmetric residue-free remove. `pinned_version 3.22.3`, MIT. Covered by `tests/bats/62-catalog-firecrawl-mcp.bats` (MCP-07).
...
| antigravity-cli | `ANTIGRAVITY_CLI_QA=1` (non-secret gate; Google auth in agent keyring) | Run `agy --print "…"` after the agent user completes Google Sign-In. Credential-free CI skips this live-auth smoke. |
...
**Minimal set to cover the cluster:** `OPENAI_API_KEY` (codex + opencode + qwen via OpenAI-compat). Antigravity requires a one-time interactive Google Sign-In for the agent user; no API key is injected by AgentLinux. Add `DASHSCOPE_API_KEY` only to test qwen against native Qwen models.
...
| AGT-06 | Phase 24 | antigravity-cli | Done |
| AGT-05 | Phase 25 | opencode | Done |
| AGT-08 | Phase 26 | qwen-code | Done |
| WORK-01 | Phase 27 | ccusage | Done |
| ENABLE-08 | Phases 24–26 | passive autoupdate freeze (opencode/qwen-code; Antigravity opt-out provisioned) 🔧 | Done |
...
| MCP-07 | Phase 40 | firecrawl-mcp — hosted OAuth bare-URL (ADR-017) | ✓ Covered (Docker 2/2 green) |
