...
| CAT-05 | Phase 6 | ~ In progress (06-01 scripts/build-release.sh emits dist/catalog-<tag>.json byte-stable via `cp` + 10-installer.bats CAT-05 @tests enforce byte-stability of staged snapshot; 06-04 release.yml build-step `test -s dist/catalog-${TAG}.json` presence gate + files: glob publishes `dist/catalog-*.json` alongside tarball + .sha256 via softprops@v2.6.2. End-to-end release-sibling upload deferred to first v0.3.0-rc1 tag push per 06-VALIDATION Manual-Only row 3) |
...
| INST-03 | Phase 6 | ~ In progress (06-01 scripts/build-release.sh emits reproducible tarball + SHA256 sidecar; 06-02 packaging/curl-installer/install.sh verifies SHA256 over HTTPS before extract + tests/bats/60-curl-installer.bats exercises the SHA256-gate path; 06-04 release.yml build-step verify sha256sum -c round-trip + publish files glob covers `agentlinux-*.tar.gz` + `agentlinux-*.tar.gz.sha256` via softprops@v2.6.2. End-to-end `curl \| sudo bash` against a real GH Release asset deferred to first v0.3.0-rc1 tag push per 06-VALIDATION Manual-Only row 3) |
...
| TST-05 | Phase 6 | ~ In progress (05-01 tests/bats/51-agt02-release-gate.bats authors the canonical AGT-02 release-gate @test; 06-04 release.yml gate-2-docker runs it inside Docker × {22.04,24.04} AND gate-3-qemu runs it inside QEMU × {22.04,24.04} — AGT-02 red in either runtime blocks downstream build/publish via explicit `needs:` chain. End-to-end tag-push exercise deferred to first v0.3.0-rc1 per 06-VALIDATION Manual-Only row 3) |
| TST-08 | Phase 6 | ~ In progress (Phase 4+5 Docker bats exercise the pinned catalog combo end-to-end via tests/docker/run.sh ubuntu-24.04 — every catalog agent installed at its `pinned_version` + tests/bats/50-agents.bats + tests/bats/51-*.bats; 06-04 release.yml gate-4-pinned-combo re-runs that pipeline as a distinct observable gate in the Actions UI — needs gate-3-qemu so red blocks build/publish. End-to-end release-gate exercise deferred to first v0.3.0-rc1 tag push per 06-VALIDATION Manual-Only row 3) |
| DOC-01 | Phase 6 | ✓ Complete (06-05 docs: README.md 138-line Pattern 7 landing page with Install/Verify/Uninstall/Stability sections + version stamp `<!-- VERSION_START -->v0.3.0<!-- VERSION_END -->` + canonical `curl -fsSL https://agentlinux.org/install.sh \| sudo bash` verbatim; docs/STABILITY-MODEL.md 124-line user-facing ADR-011 companion with v0.3.0 pins 2.1.98 + 1.37.1 + 1.59.1 + 3-way divergence semantics + `agentlinux upgrade` worked example + `agentlinux pin` escape hatch. TST-07 phase-close audit emitted `TST-07 gate: GREEN` for all 6 Phase 6 req IDs; see 06-05-AUDIT.md.) |
