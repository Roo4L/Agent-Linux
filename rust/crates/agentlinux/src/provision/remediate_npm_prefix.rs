...
fn apply_rebase(ctx: &ProvisionCtx, old_prefix: &str, old_owner: &str) -> io::Result<()> {
    ...
    if let Err(e) = sysio::ensure_line_in_owned_file(
    ...
    // REPLACE any earlier `prefix=` line rather than leaving two. npm honours the
    // LAST prefix line while `probe::effective_npm_prefix` reads the last as well,
    // so the two agree — but a stale first line is still a lie to anyone reading
    // the file, and an earlier cut of this code appended without replacing and
    // re-ran the whole module migration on every provision, forever.
    ...
    let (mut migrated, mut failed, mut skipped) = (0u32, 0u32, 0u32);
    if modules.is_empty() {
        crate::plog!(
        ...
        let tally = migrate_modules(ctx, user, &modules, MIGRATION_BUDGET, &mut |m| {
            crate::provision::log::line(m);
        });
    ...
    // `skipped` sits in the summary alongside the other two because this is the
    // line an operator reads to decide whether the old prefix is safe to delete.
    // Reporting only migrated+failed makes a budget-truncated run look complete.
    crate::plog!(
        "[REMEDIATE-01] rebase complete: migrated={migrated} failed={failed} \
         skipped={skipped} old_prefix={old_prefix} (NOT deleted; user cleanup)"
    );
    Ok(())
}
...
/// What one rebase's module migration did. A named struct rather than a tuple
/// because `skipped` joined it late: `(migrated, failed)` read fine at both call
/// sites and `(migrated, failed, skipped)` would not have.
struct MigrationTally {
    migrated: u32,
    failed: u32,
    /// Modules the aggregate budget stopped us from attempting at all. Distinct
    /// from `failed`: nothing was tried, so nothing is half-done.
    skipped: u32,
}
...
fn migrate_modules(
    ...
    // A per-item bound is not a bound on the loop. Each `npm install -g` is capped
    // at PER_MODULE_TIMEOUT_MS, but the item COUNT comes from host data — 40
    // globals on a brownfield host is a 3.3-hour ceiling inside one provisioner
    // step. ADR-022 §4 deleted the generic package retry for exactly this shape (a
    // per-command bound silently multiplied into a much larger real one); the
    // multiplier being host-supplied rather than a constant makes it worse.
    //
    // The migration is explicitly best-effort and the OLD prefix is never deleted,
    // so stopping early is safe: what is left behind is exactly what was left
    // behind before this step ran, and the operator is told what was not attempted.
    ...
    for pkg_at_ver in modules {
        ...
        let argv: Vec<String> = ["npm", "install", "-g", "--", pkg_at_ver]
            ...
            .map(|s| s.to_string())
            .collect();
        // Bound each install at the smaller of the per-module cap and whatever is
        // left of the aggregate budget, so the last module cannot overshoot.
        let remaining = deadline.saturating_duration_since(std::time::Instant::now());
        let bound = remaining.as_millis().min(u128::from(PER_MODULE_TIMEOUT_MS)) as u64;
        let r = (ctx.fx.as_user)(user, &argv, &[], Capture::Buffered, Some(bound));
        if r.exit_code == 0 {
    ...
    if skipped > 0 {
        ...
        log(&format!(
            "[REMEDIATE-01:partial] {skipped} module(s) NOT attempted — the {}s \
             migration budget expired. The old prefix is intact; re-run provision to \
             continue, or migrate them by hand.",
            budget.as_secs()
        ));
    }
    MigrationTally {
...
mod remediate_npm_prefix_tests {
    ...
    /// The aggregate budget is the difference between a bounded step and a
    /// per-item bound multiplied by a host-supplied count. An exhausted budget
    /// must attempt NOTHING further and must SAY how much it skipped: a silent
    /// cap reads as "everything was migrated", and the operator then deletes an
    /// old prefix that still holds modules.
    #[test]
    fn an_exhausted_budget_attempts_nothing_and_says_how_much_it_skipped() {
        let d = TempDir::new().unwrap();
        let ctx = rebase_ctx(d.path(), npm_two_modules);
        let modules = vec!["a@1.0.0".to_string(), "b@2.0.0".to_string()];

        let mut lines = Vec::new();
        let tally = migrate_modules(
            &ctx,
            FIXTURE_USER,
            &modules,
        ...
        assert_eq!((tally.migrated, tally.failed, tally.skipped), (0, 0, 2));
        assert!(
            npm_calls().is_empty(),
            "an expired budget must not dispatch npm at all, got {:?}",
            npm_calls()
        );
        assert_eq!(lines.len(), 1, "one summary line, not one per module");
        assert!(
            lines[0].contains("2 module(s) NOT attempted"),
            "the operator must be told the COUNT, got {:?}",
            lines[0]
        );
    }
