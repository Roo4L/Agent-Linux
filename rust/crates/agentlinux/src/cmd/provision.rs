...
/// `pkill`/`userdel` unconditionally with no injection point, so even the
/// leftovers verdict above is only reachable from a test by tearing down a real
/// host. Skipped with a back-reference rather than left to redden the enforcing
/// gate for anyone whose diff lands here (ADR-020 §4, third case).
#[cfg_attr(test, mutants::skip)]
...
fn report_only(user: &str, home: &str, distro: &distro::Distro, format: Option<&str>) -> ExitCode {
    let (mut out, mut err) = (io::stdout(), crate::provision::log::err_sink());
    report_only_to(
        &mut Out {
            out: &mut out,
            err: &mut err,
...
mod provision_tests {
    ...
    fn the_install_path_runs_its_phases_in_order() {
        ...
        assert_eq!(
            taken(),
            vec![
                // FIRST, ahead of every decision that can end the run. A transcript
                // opened after the bail flush recorded nothing for the outcomes an
                // operator actually needs it for — a refused brownfield host, an
                // unsupported distro, the alt-user gate — and left the previous
                // run's file in place to be misread as this one's.
                "log_init",
                "default_user",
                "check_adoptable",
                "detect_distro",
            ...
            "the transcript must open BEFORE anything that can bail, the re-scan \
             must follow the steps, and adoption must follow the re-scan"
        );
    }
    ...
    fn an_alt_user_name_that_is_not_a_legal_user_is_refused() {
        ...
        for bad in ["", "root", "Bad User!"] {
            ...
            let seq = taken();
            assert!(
                !seq.contains(&"probe_facts"),
                "nothing may proceed on an illegal alt user, seq={seq:?}"
            );
            // The transcript IS expected here: a refusal an operator has to
            // diagnose is exactly what the transcript exists for, and opening it
            // is not a mutation of anything the run promised to leave alone.
            assert!(seq.contains(&"log_init"), "seq={seq:?}");
        }
    }
    ...
    fn purge_returns_before_anything_else_is_touched() {
        // --purge is a teardown: it must not detect the distro, probe or run a
        // step. It DOES open a transcript — a teardown that removes an account
        // and a tree is the run you most want a record of, and it had none.
        let _lock = crate::test_support::EnvScope::new();
        let _ = taken();
        let mut args = base_args();
        args.purge = true;
        ...
        assert_eq!(
            taken(),
            vec!["log_init", "default_user", "check_adoptable", "purge"]
        );
    }
    ...
    #[test]
    fn dry_run_returns_before_deciding_and_never_opens_a_transcript() {
        // UX-01: --dry-run reports every decision the real run would make — but
        // `emit_report` computes those itself and `dry_run_report` takes no
        // resolutions, so the report does not need the DECIDE phase. Returning
        // BEFORE it is what keeps a preview from reaching `decide_core_with`'s
        // consent prompt and blocking an unattended orchestration step forever.
        let _lock = crate::test_support::EnvScope::new();
        let _ = taken();
        let mut args = base_args();
        args.dry_run = true;
    ...
    #[test]
    fn a_bail_stops_before_any_step() {
        ...
        assert_eq!(
            taken(),
            vec![
                // The transcript opens FIRST now. A bail used to produce none at
                // all, so the one outcome an operator has to diagnose left no
                // record — and the previous run's file in its place.
                "log_init",
                "default_user",
                "check_adoptable",
                "detect_distro",
            ...
            "no step and no scan may run after a bail"
        );
    }
