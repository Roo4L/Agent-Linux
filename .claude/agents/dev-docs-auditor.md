...
description: Reviews changes under product/plugin/bin/, product/plugin/lib/, product/plugin/provisioner/, product/plugin/cli/src/, product/plugin/catalog/, and product/packaging/curl-installer/ to verify the matching docs/internals/<component>.md is still accurate. Flags missing component docs, stale claims, source-line deep links, and TOC drift in docs/internals/README.md. Read-only — main agent triages.
...
- Any change under `product/plugin/bin/agentlinux-install`.
- Any change under `product/plugin/lib/*.sh` (logging, idempotency, as_user, distro detection).
- Any change under `product/plugin/provisioner/*.sh` (agent user, sudoers drop-in, Node.js, PATH wiring, registry CLI).
- Any change under `product/plugin/cli/src/**` (the registry CLI source).
- Any change under `product/plugin/catalog/{schema,catalog}.json` or `product/plugin/catalog/agents/<name>/{install,uninstall}.sh`.
- Any change under `product/plugin/catalog/lib/` (shared catalog recipe helpers).
- Any change under `product/packaging/curl-installer/install.sh`.
- When a **new top-level component surface lands** under `product/plugin/` (a new provisioner, a new CLI command class, a new catalog backend) — the matching `docs/internals/<component>.md` should ship in the same PR.
...
- `product/tests/` changes that don't touch `product/plugin/` source paths above.
...
- **A new component shipped without a doc.** When a new top-level surface lands under `product/plugin/` (new provisioner, new CLI command class, new catalog backend), the matching `docs/internals/<surface>.md` should ship in the same PR. Flag.
...
Files reviewed: product/plugin/provisioner/30-nodejs.sh, product/plugin/provisioner/40-path-wiring.sh
...
This auditor does NOT spawn other reviewers (per ADR-010 — the review loop's dispatcher is the main agent), and it does NOT block delivery. The dev-docs are reference material; staleness is a flag for the main agent to triage, not a release blocker.
