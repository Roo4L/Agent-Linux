...
- When a `sudoers` drop-in is written or modified, is it installed mode `0440`
  (`root:root`, group- and other-readable but not writable) as `visudo`/`sudo`
  require, and does its `NOPASSWD` grant stay as narrowly scoped as the task
  allows rather than a blanket `ALL=(ALL) NOPASSWD: ALL` (per ADR-012)? A
  drop-in that is group- or world-writable, or wider in scope than needed, is a
