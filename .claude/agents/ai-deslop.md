---
name: ai-deslop
description: Reviews changed code for AI-generated slop — tour-guide comments, defensive try/catch wrapping trusted internal calls, `any`-cast workarounds, redundant validation past typed boundaries, backwards-compat shims for code with no external consumers, stylistic inconsistency with the surrounding file, filler doc prose, and dead test scaffolding. Use on any change to product/plugin/, product/tests/ (excluding bats spec assertions), product/packaging/, and docs/ (excluding ADRs and research summaries).
...
- Any source file under `product/plugin/` (bash, TypeScript, JSON catalog).
- Any file under `product/tests/docker/`, `product/tests/qemu/`, `product/tests/harness/`, or
  `product/tests/bats/helpers/`. **Skip** the bats spec files themselves
  (`product/tests/bats/*.bats`) — they are the behaviour contract and their
  assertion style is intentional.
- Any file under `product/packaging/` (the curl-installer, the .deb wrapper).
- Any file under `docs/` **except** `docs/decisions/*` (ADRs are
  load-bearing prose) and `docs/research/*` (decision research relies on
  careful prose framing).
- Always skip `.planning/` (GSD workflow state, not project content) and
  vendored / generated files (`product/plugin/cli/dist/`, `node_modules/`,
  `product/tests/qemu/cloud-images/*.img`).
...
- **`echo "Done!"` at the end of every step.** Bash provisioners use
  `log_info` / `log_success` (per `product/plugin/lib/log.sh`). A bare `echo`
...
- **Type re-exports in `product/plugin/cli/src/index.ts` for internal
...
Files reviewed: product/tests/docker/dogfood.sh, product/plugin/cli/src/version.ts

Findings:
- product/tests/docker/dogfood.sh:14-18 — five-line banner comment "# ===== DOGFOOD
  HARNESS =====" with description that restates the file's purpose.
  Filename + frontmatter cover this. Delete.
- product/tests/docker/dogfood.sh:67 — `# Run the install script as the agent user`
  above `as_user agent bash -c "..."`. Tour-guide comment; the function
  call already says it. Delete.
- product/tests/docker/dogfood.sh:142 — `|| true` swallowing a `chown` failure
...
- product/plugin/cli/src/version.ts:8-12 — JSDoc block on a four-line
  `readPackageVersion()` helper. Function name is clear; rename to
  `readCliVersion()` if needed and drop the docblock.
- product/plugin/cli/src/version.ts:22 — `value as unknown as string` to silence
