...
description: Reviews AgentLinux catalog entries and per-agent install recipes for JSON Schema validity, privilege-drop correctness (as_user usage, no sudo npm install -g), symmetric uninstall paths, and absence of /usr/local shim patterns. Use on any change under product/plugin/catalog/agents/*, product/plugin/catalog/lib/, product/plugin/catalog/catalog.json, product/plugin/catalog/schema.json, or product/plugin/cli/scripts/validate-catalog.mjs.
...
- Any change under `product/plugin/catalog/agents/<name>/install.sh` or `product/plugin/catalog/agents/<name>/uninstall.sh`.
- Any change under `product/plugin/catalog/lib/` (shared recipe helpers).
- Any change to `product/plugin/catalog/catalog.json` (the embedded agent list).
- Any change to `product/plugin/catalog/schema.json` (the JSON Schema — breaking changes need an ADR).
- Any change to `product/plugin/cli/scripts/validate-catalog.mjs` (the validator that gates the pre-commit hook and CI).
...
1. **JSON Schema validity.** Every entry in `product/plugin/catalog/catalog.json` MUST validate against `product/plugin/catalog/schema.json`. Run `node product/plugin/cli/scripts/validate-catalog.mjs` locally. If schema is `additionalProperties: false` (it is), extra catalog fields are a hard fail.
2. **Privilege-drop and ownership.** Every recipe that installs an npm package must run it through the project user-scoped install contract — NEVER bare `sudo npm install -g` (the canonical AgentLinux bug class), and never a root-owned global install. Shared helpers under `product/plugin/catalog/lib/` must preserve the same contract.
...
1. `node product/plugin/cli/scripts/validate-catalog.mjs` — must exit 0.
2. `grep -nE 'sudo npm install|npm install -g' product/plugin/catalog/agents/*/install.sh` — every hit must be `as_user ... npm install -g ...`, never `sudo npm install`.
3. For each `install.sh`, check that a sibling `uninstall.sh` exists: `ls product/plugin/catalog/agents/<name>/{install,uninstall}.sh`.
4. `grep -n '/usr/local' product/plugin/catalog/agents/*/install.sh` — should return zero lines.
5. `grep -n 'curl .* | bash\|curl .* | sh' product/plugin/catalog/agents/*/install.sh` — any hit is a flag.
...
Files reviewed: product/plugin/catalog/agents/claude-code/install.sh, product/plugin/catalog/catalog.json

Validator: `node product/plugin/cli/scripts/validate-catalog.mjs` → exit 0 (all recipes valid).

Findings:
- product/plugin/catalog/agents/claude-code/install.sh:14 — `sudo npm install -g @anthropic-ai/claude-code`. Critical. Replace with `as_user agent npm install -g @anthropic-ai/claude-code`. This is the bug class AgentLinux exists to eliminate.
- product/plugin/catalog/agents/claude-code/install.sh:20 — writes wrapper to `/usr/local/bin/claude`. Remove — agent's `~/.npm-global/bin/claude` is already on PATH via the Phase 2 profile.d wiring.
- product/plugin/catalog/agents/claude-code/ — no `uninstall.sh`. Symmetric uninstall is required (CLI-04 / INST-04). Add one that runs the user-scoped npm uninstall.
- product/plugin/catalog/catalog.json — missing the post-install verification field. `agentlinux list` cannot render installed indicator.
