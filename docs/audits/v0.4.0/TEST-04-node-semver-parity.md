...
| 4 | **Compound (space, closed)** | `>=2.0.0 <3.0.0` (every `compatibility_window`, e.g. `claude-code`) | space-separated accepted | **`VersionReq::parse` ERRORS on the space** | `normalize_range` rewrites space → `", "` before parse (`semver_shim.rs:54`) |
| 5 | **Compound (open-ended)** | `>=2026.2.17` (`slack-mcp`); `>=0.37.0 <1.0.0` (`sentry-mcp`) | accepted | single comparator parses; still no space to split | `normalize_range` (no-op for a lone comparator; splits the closed form) |
