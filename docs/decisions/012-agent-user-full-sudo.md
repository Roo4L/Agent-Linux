...
- **The canonical acceptance test is unaffected.** Claude Code self-updating without sudo and without EACCES is about the user-owned path, not about whether sudo exists on the system. `claude update` still runs in the user-owned prefix; sudo never enters the self-update path.
