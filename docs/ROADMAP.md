...
milestone — the strategy / roadmap framing — closes once the
...
1. Finish v0.3.3 — the strategy / roadmap split and the website
   refresh.
...
We carry an opportunistic security-hardening theme from the
...
The differentiators: `bare` / `must-haves` / `optimum` presets,
