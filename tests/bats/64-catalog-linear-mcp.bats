...
@test "MCP-09: linear-mcp registers a BARE remote URL (no credential, ADR-017) into every installed agent, then deregisters with no residue" {
  ...
  _assert_present_if_installed agy "jq -e --arg u \"${url}\" '.mcpServers[\"linear-mcp\"] | .serverUrl==\$u and (has(\"headers\")|not)' /home/agent/.gemini/config/mcp_config.json"
  ...
  _assert_gone_if_present agy "jq -e '.mcpServers | has(\"linear-mcp\")' /home/agent/.gemini/config/mcp_config.json"
