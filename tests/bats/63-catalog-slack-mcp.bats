...
#
# THIN INSTALLER (ADR-018): slack-mcp registers Slack's OFFICIAL hosted remote MCP
...
@test "MCP-08: slack-mcp registers a BARE remote URL (no credential, ADR-018) into every installed agent, then deregisters with no residue" {
  ...
  [[ "${status}" -ne 0 ]] \
    || __fail "MCP-08" "NO credential in any agent config (thin installer, ADR-018)" "credential-shaped string found" "$LOG"
...
@test "MCP-08: slack-mcp entry shape — official hosted remote-http, thin installer, no package license" {
  ...
  # auth), NO secret_env (ADR-018), and NO license (Slack's proprietary hosted
